...
export function FrameworkConfigDialog({ open, onOpenChange }: FrameworkConfigDialogProps) {
  const { toast } = useToast();
  const [configData, setConfigData] = useState<any>(null);

  // Fetch current framework config
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Framework Configuration</DialogTitle>
          <DialogDescription>
            Edit platform-wide configuration (pipeline, sources, buffers). Only values can be edited - structure is read-only. 
            Changes apply to ALL devices globally.
          </DialogDescription>
        </DialogHeader>
