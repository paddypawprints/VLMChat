#!/usr/bin/env python3
"""
Test the refactored Environment with dictionary-based key-value store.
"""
import sys
...
def test_basic_operations():
    ...
    assert "Camera+cam1+current_image" in env
    assert not env.has("NonExistent+key")
    
    # Get with default
    assert env.get("NonExistent+key", "default") == "default"
    
    # Remove
    assert env.remove("ObjectDetector+det1+results") == True
    assert env.remove("NonExistent+key") == False
...
def test_full_key_usage():
    """Test using full keys for common use cases."""
    print("\nTesting full key usage patterns...")
    env = Environment.get_instance()
    env.clear()
    
    # Application keys
    env.set("App+main+current_image", "test_image")
    assert env.get("App+main+current_image") == "test_image"
    
    env.set("App+main+history", {"history": "data"})
    assert env.get("App+main+history") == {"history": "data"}
    
    # Verify keys are stored correctly
    assert "App+main+current_image" in env.keys()
    assert "App+main+history" in env.keys()
    
    print("  ✓ Application keys work with full path")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("  • Full key access (clean, explicit design)")
        print("  • General-purpose extensible design")
        sys.exit(0)
