...
An edge AI platform for deploying vision-language models (VLMs) on edge devices with a web-based management interface.
...
│
└── device-sdk/          # 📦 Generated Python SDK (TBD)
    └── edge_llm_client/ # Auto-generated from shared/
```
...
- Real-time chat with edge AI models
- Detection alerts and watchlist management
- MQTT communication with devices
- PostgreSQL database for persistence

### Shared Contracts (shared/)
Single source of truth for all APIs and data formats:
- Versioned JSON schemas
- OpenAPI specifications for REST APIs
- AsyncAPI specifications for MQTT/WebSocket

## Key Features
- Multimodal chat using SmolVLM via Hugging Face Transformers
- Optional ONNX Runtime optimization for faster inference
- Picamera2-based camera integration on compatible hardware
- Web-based device management and monitoring
- MQTT messaging for real-time device communication
- Zero incremental cost - all processing on edge devices
