...
class CocoCategory(Enum):
    ...
    The member name is a valid Python identifier (e.g., STOP_SIGN),
    and the value is a tuple (id, "label_string").
    """
    ...
    def __init__(self, id: int, label: str):
        """Custom init to set id and label properties."""
        self.id = id
        self.label = label
    ...
    def from_string(cls, value: str) -> Optional['CocoCategory']:
        """
        Gets an enum member from its string value, or None if not found.
        """
        for member in cls:
    ...
    def from_id(cls, id_value: int) -> Optional['CocoCategory']:
        """
        Gets an enum member from its integer ID, or None if not found.
        """
        for member in cls:
