...
class FashionClipVisionTask(BaseTask):
    ...
    def __init__(self, fashion_clip_model=None, task_id: str = "fashion_clip_vision", image_format: str = "pil"):
        """
        ...
        # Select backend based on required image format (FashionClip only has OpenCLIP backend currently)
        if self.fashion_clip_model is not None:
            if self.image_format == "pil":
                self.fashion_clip_model.set_runtime("openclip")
            else:
                raise ValueError(f"Unsupported image format: {self.image_format}. FashionClip only supports 'pil'.")
        ...
        # Query backend's native format
        from ..image_format import ImageFormat
        if self.fashion_clip_model and hasattr(self.fashion_clip_model, '_runtime') and hasattr(self.fashion_clip_model._runtime, 'native_image_format'):
            backend_format = self.fashion_clip_model._runtime.native_image_format
            format_map = {
                "pil": ImageFormat.PIL,
...
if __name__ == "__main__":
    ...
    class MockFashionClipModel:
        ...
        def encode_image(self, image: Image.Image) -> 'MockTensor':
            ...
            return torch.randn(1, 768)  # type: ignore[return-value]
