...
class ImageViewerTask(BaseTask):
    ...
    def run(self, context: Context) -> Context:
        ...
        if isinstance(image_data, list) and image_data:
            image_data = image_data[0]  # Unwrap nested list
        
        # Extract from ImageContainer if needed
        from ..image_container import ImageContainer
        from ..image_format import ImageFormat
        if isinstance(image_data, ImageContainer) and self.native_input_format:
            image = image_data.get_format(self.native_input_format)
            if image is None:
                # Fallback to any available format
                for fmt in image_data.get_cached_formats():
                    image = image_data.get_format(fmt)
        ...
        if isinstance(image, Image.Image):
            ...
            # Fallback: assume it's array-like and convert
