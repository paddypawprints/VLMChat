...
class DiagnosticTask(BaseTask):
    ...
    def __init__(
        ...
        self.exit_code_to_set = exit_code  # Exit code to set in context
        self.raise_error = False  # For testing exception handling
        self.error_type = "RuntimeError"  # Type of exception to raise
    ...
    def configure(self, **kwargs):
        ...
        if 'exit_code' in kwargs:
            self.exit_code_to_set = int(kwargs['exit_code'])
        if 'raise_error' in kwargs:
            self.raise_error = kwargs['raise_error'] in (True, 'true', 'True', '1', 1)
    ...
    def run(self, context: Context) -> Context:
        ...
        # Raise exception if configured (for testing)
        if self.raise_error:
            # Try to get exception class from builtins first, then globals
            import builtins
            exc_class = getattr(builtins, self.error_type, None) or globals().get(self.error_type, RuntimeError)
            if not isinstance(exc_class, type):
                exc_class = RuntimeError
            raise exc_class(f"Diagnostic task '{self.message}' raising {self.error_type}")
        ...
        # Set exit code for loop condition testing
        self.exit_code = self.exit_code_to_set
